 ...
 * G30 - Single Z Probe, probes bed at given XY location.
...
inline void gcode_M119() {
  SERIAL_PROTOCOLPGM("{\"endstops\":[");
  bool firstOccurrence = true;
  #if HAS_X_MIN
    SERIAL_PROTOCOL((READ(X_MIN_PIN)^X_MIN_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  #if HAS_X_MAX
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(X_MAX_PIN)^X_MAX_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  #if HAS_Y_MIN
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(Y_MIN_PIN)^Y_MIN_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  #if HAS_Y_MAX
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(Y_MAX_PIN)^Y_MAX_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  #if HAS_Z_MIN
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
  ...
  #if HAS_Z_MAX
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(Z_MAX_PIN)^Z_MAX_ENDSTOP_INVERTING));
    firstOccurrence = false; 
  #endif
  #if HAS_Z2_MAX
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(Z2_MAX_PIN)^Z2_MAX_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  #if HAS_Z_PROBE
    if(!firstOccurrence)  SERIAL_PROTOCOLPGM(",");
    SERIAL_PROTOCOL((READ(Z_PROBE_PIN)^Z_PROBE_ENDSTOP_INVERTING));
    firstOccurrence = false;
  #endif
  SERIAL_PROTOCOLLN("]}");
}
